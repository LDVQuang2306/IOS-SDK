...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // 1. Tính toán vùng bao quanh tổng thể
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(slider_width, label_size.y + 16));
    
    // 2. Tính toán vùng hiển thị thanh trượt (Vùng Visual - Giữ nguyên để vẽ không bị to ra)
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 10), total_bb.Max);

    // 3. [MỚI] Tạo vùng tương tác ảo (Hitbox) rộng hơn vùng vẽ
    // Mở rộng chiều cao (Y) ra mỗi bên 8px để dễ bắt chuột hơn
    ImRect interact_bb = frame_bb;
    interact_bb.Expand(ImVec2(0.0f, 8.0f)); 

    ItemSize(total_bb, style.FramePadding.y);
    
    // Dùng interact_bb để đăng ký item thay vì frame_bb
    if (!ItemAdd(total_bb, id, &interact_bb))
        return false;
    ...
    // Dùng interact_bb để kiểm tra Hover/Click
    const bool hovered = ItemHoverable(interact_bb, id);
    const bool clicked = (hovered && g.IO.MouseClicked[0]);
    if (clicked)
    {
        SetActiveID(id, window);
    ...
    ImRect grab_bb;
    // Truyền interact_bb vào SliderBehavior để tính toán logic kéo thả trên vùng rộng hơn
    const bool value_changed = SliderBehavior(interact_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    // --- PHẦN VẼ (RENDER) ---
    // Vẫn dùng frame_bb gốc để vẽ background (để thanh slider trông vẫn mỏng đẹp)
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 15, 16), 5.0f);
    
    // Tính lại tọa độ Y trung tâm của frame_bb để vẽ Thumb cho chuẩn.
    // Vì grab_bb bây giờ được tính dựa trên interact_bb (cao hơn), nên nếu dùng grab_bb.Min.y trực tiếp
    // thì cục thumb có thể bị lệch vị trí. Ta lấy X của grab_bb nhưng Y cố định theo frame_bb.
    float thumb_center_y = frame_bb.GetCenter().y;

    // Vẽ phần đã trượt (Active bar)
    // Lưu ý: grab_bb.Min.x là vị trí chính xác của con trượt theo logic
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(195, 143, 255), 5.0f);
    
    // Vẽ cục Thumb (Con trượt)
    // Dùng thumb_center_y để đảm bảo nó luôn nằm giữa thanh trượt về mặt hình ảnh
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x - 1, thumb_center_y), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);
