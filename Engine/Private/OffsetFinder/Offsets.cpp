...
* All code is copied through the kernel, never read directly. Returns the VTable index or -1.
*/
static int32 FindProcessEventByFunctionFlagTests(void** Vft)
{
...
void Off::InSDK::ProcessEvent::InitPE()
{
	...
	void** Vft = static_cast<void**>(FirstObj.GetVft());
	...
	int32_t FoundIdx = Off::UFunction::FunctionFlags > 0 ? FindProcessEventByFunctionFlagTests(Vft) : -1;

	if (FoundIdx >= 0)
	{
		LogInfo("InitPE: FunctionFlags test heuristic -> 0x%X", FoundIdx);
	}
	else
	{
		/* Fallback: fingerprint scorer (UObject::Index, FUObjectItem stride, UStruct/UFunction field loads) */
		FoundIdx = Architecture_x86_64::FindProcessEventIndex(Vft);
	}

	const void* FoundPtr = (FoundIdx >= 0) ? SafeRead<void*>(reinterpret_cast<uintptr_t>(Vft + FoundIdx)) : nullptr;
