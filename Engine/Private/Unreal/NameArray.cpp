 ...
 *
 * A pool initialized with InitWithKnownNamePoolLayout (Delta Force) has a verified CurrentBlock field. */
int32 NameArray::GetNumChunks()
{
    if (!GNames)
        return 0;

    if (!Settings::Internal::bUseNamePool || bHasKnownNamePoolLayout)
        return SafeRead<int32>(reinterpret_cast<uintptr>(GNames + Off::NameArray::MaxChunkIndex));

    const uintptr Blocks = reinterpret_cast<uintptr>(GNames + Off::NameArray::ChunksStart);

    constexpr int32 FNameMaxBlocks = 0x2000; // UE source constant
    int32 LastValid = -1;
    for (int32 i = 0; i < FNameMaxBlocks; ++i)
    {
        const uintptr Block = SafeRead<uintptr>(Blocks + i * sizeof(void*));
        if (!IsPlausiblePointer(Block) || IsBadReadPtr(Block))
            break;
        LastValid = i;
    }
    return LastValid;
...
int32 NameArray::GetByteCursor()
{
    ...
    if (bHasKnownNamePoolLayout)
        return SafeRead<int32>(reinterpret_cast<uintptr>(GNames + Off::NameArray::ByteCursor));

    const int32 CurrentBlock = GetNumChunks();
    if (CurrentBlock < 0)
        return 0;

    uint8_t* BlockPtr = reinterpret_cast<uint8_t*>(SafeRead<uintptr>(reinterpret_cast<uintptr>(GNames + Off::NameArray::ChunksStart) + CurrentBlock * sizeof(void*)));
    if (!IsPlausiblePointer(BlockPtr) || IsBadReadPtr(BlockPtr))
        return 0;
    ...
    {
        const uint16_t Header = SafeRead<uint16_t>(reinterpret_cast<uintptr>(BlockPtr + Cursor));
        if (Header == 0)
            break;
