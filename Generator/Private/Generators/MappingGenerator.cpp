...
std::stringstream MappingGenerator::GenerateFileData()
{
	...
	if constexpr (Settings::Debug::bShouldPrintMappingDebugData)
		LogError("%s", fmt::format("MappingGeneration: NameCounter = 0x{0:X} (Dec: {0})\n", static_cast<uint32>(NameCounter)).c_str());
	...
	if constexpr (Settings::Debug::bShouldPrintMappingDebugData)
		LogError("%s", fmt::format("MappingGeneration: NumEnums = 0x{0:X} (Dec: {0})\n", static_cast<uint32>(NumEnums)).c_str());
	...
	if constexpr (Settings::Debug::bShouldPrintMappingDebugData)
		LogError("%s", fmt::format("MappingGeneration: NumStructsAndClasse = 0x{0:X} (Dec: {0})\n\n", static_cast<uint32>(NumStructsAndClasse)).c_str());
...
void MappingGenerator::GenerateFileHeader(StreamType& InUsmap, const std::stringstream& Data)
{
	...
	if constexpr (Settings::Debug::bShouldPrintMappingDebugData)
	{
		LogError("%s", fmt::format("MappingGeneration: CompressedSize = 0x{0:X} (Dec: {0})\n", CompressedSize).c_str());
		LogError("%s", fmt::format("MappingGeneration: DecompressedSize = 0x{0:X} (Dec: {0})\n\n", UncompressedSize).c_str());
	}
